...
const sampleSurveyData: SurveyPoint[] = [
    { md: 0, incl: 0, azim: 0, tvd: 0 },
    { md: 1000, incl: 2, azim: 45, tvd: 999 },
    { md: 2000, incl: 5, azim: 50, tvd: 1996 },
    { md: 3000, incl: 8, azim: 55, tvd: 2989 },
    { md: 4000, incl: 12, azim: 60, tvd: 3975 },
    { md: 5000, incl: 15, azim: 65, tvd: 4952 },
    { md: 6000, incl: 18, azim: 70, tvd: 5917 },
    { md: 7000, incl: 20, azim: 75, tvd: 6868 },
    { md: 8000, incl: 22, azim: 80, tvd: 7803 },
    { md: 8500, incl: 23, azim: 85, tvd: 8263 }
];

const SurveyTab: React.FC<SurveyTabProps> = ({ surveyData, setSurveyData, showNotification }) => {

    const addRow = useCallback(() => {
        setSurveyData(prev => [...prev, { md: 0, incl: 0, azim: 0, tvd: 0 }]);
    }, [setSurveyData]);
                    ...
                    <tbody>
                        {surveyData.map((row, index) => (
                            <tr key={index}>
                                ...
                                <td><input type="number" value={row.incl} onChange={(e) => handleInputChange(index, 'incl', e.target.value)} step="0.1" className="w-full bg-gray-600 border border-gray-500 rounded px-1 py-0.5 text-white text-xs" /></td>
                                <td><input type="number" value={row.azim} onChange={(e) => handleInputChange(index, 'azim', e.target.value)} step="0.1" className="w-full bg-gray-600 border border-gray-500 rounded px-1 py-0.5 text-white text-xs" /></td>
