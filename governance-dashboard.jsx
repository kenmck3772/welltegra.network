...
import {
  ...
  Terminal, Gauge, Clock, Eye, XCircle, AlertOctagon
} from 'lucide-react';

// ============================================
// GOVERNANCE & MISSION CONTROL DASHBOARD
// The Brahan Engine - Digital Offshore Intervention
// ============================================

const TEAM_MEMBERS = [
  { id: 1, name: 'Ken McKenzie', role: 'Project Director', discipline: 'Digital' },
  { id: 2, name: 'Sarah Chen', role: 'Subsea Engineer', discipline: 'Subsea' },
  { id: 3, name: 'James Morrison', role: 'HSE Lead', discipline: 'Safety' },
  { id: 4, name: 'Dr. Aisha Okafor', role: 'Petroleum Engineer', discipline: 'Reservoir' },
  { id: 5, name: 'Lars Andersen', role: 'Drilling Supervisor', discipline: 'Drilling' },
  { id: 6, name: 'Maria Rodriguez', role: 'Coiled Tubing Specialist', discipline: 'CT' },
  { id: 7, name: 'David Thomson', role: 'Well Integrity Lead', discipline: 'Integrity' },
  { id: 8, name: 'Priya Sharma', role: 'Data Scientist', discipline: 'AI/ML' },
  { id: 9, name: 'Tom O\'Brien', role: 'Rig Superintendent', discipline: 'Operations' },
  { id: 10, name: 'Fatima Al-Rashid', role: 'Geologist', discipline: 'Geology' },
  { id: 11, name: 'Robert MacLeod', role: 'Completions Engineer', discipline: 'Completions' }
];
...
const CONFLICT_SCENARIOS = [
  {
    ...
    aiRecommendation: 'ROP: 120 ft/hr (Performance KPI)',
    safetyOverride: 'ROP: 85 ft/hr (Safety First)',
    riskScore: 7.2,
    performanceImpact: '-29% ROP',
    resolved: false
  },
  {
    id: 'pressure-anomaly',
    ...
    aiRecommendation: 'Continue operations (98% confidence)',
    safetyOverride: 'Pause & investigate pressure trend',
    riskScore: 5.8,
    performanceImpact: '-2 hrs downtime',
    resolved: false
  }
];
...
const DashboardApp = () => {
  ...
  const [operationalMode, setOperationalMode] = useState('AI_DRIVEN'); // AI_DRIVEN | MANUAL_OVERRIDE | LOCAL_MODE
  const [activeConflicts, setActiveConflicts] = useState([]);
  const [teamStatuses, setTeamStatuses] = useState(
    TEAM_MEMBERS.map(member => ({
      ...member,
      status: 'active', // active | advisory | vetoed
      lastUpdate: Date.now()
    }))
  );
  ...
  useEffect(() => {
    const telemetryInterval = setInterval(() => {
      // Simulate DCI fluctuations
      setDciScore(prev => {
        const change = (Math.random() - 0.5) * 8;
        const newScore = Math.max(40, Math.min(100, prev + change));

        // Trigger alert if DCI drops below 60
        if (newScore < 60 && prev >= 60) {
          setSystemAlerts(alerts => [
            ...alerts,
            {
              id: Date.now(),
              type: 'warning',
              message: 'DCI below 60% - Manual Override Required',
      ...
      setTelemetryLatency(prev => {
        ...
        const newLatency = spike
          ? Math.random() * 1000 + 1200 // 1200-2200ms spike
          : Math.random() * 400 + 200;   // 200-600ms normal

        // Switch to LOCAL_MODE if latency exceeds 1500ms
        if (newLatency > 1500) {
          setOperationalMode('LOCAL_MODE');
          setSystemAlerts(alerts => [
            ...alerts,
            {
              id: Date.now(),
              type: 'critical',
              message: 'LATENCY SPIKE - Switched to LOCAL MODE (Onsite Authority)',
              timestamp: new Date().toLocaleTimeString()
            }
          ]);
        } else if (prev > 1500 && newLatency <= 1500) {
          setOperationalMode(dciScore >= 60 ? 'AI_DRIVEN' : 'MANUAL_OVERRIDE');
        }
    ...
    const conflictInterval = setInterval(() => {
      if (Math.random() < 0.1 && activeConflicts.length < 2) {
        const availableConflicts = CONFLICT_SCENARIOS.filter(
          scenario => !activeConflicts.find(c => c.id === scenario.id)
        );
        if (availableConflicts.length > 0) {
          const newConflict = availableConflicts[Math.floor(Math.random() * availableConflicts.length)];
          setActiveConflicts(prev => [...prev, { ...newConflict, triggeredAt: Date.now() }]);
          setSystemAlerts(alerts => [
            ...alerts,
            {
              id: Date.now(),
              type: 'warning',
              message: `Specialist Conflict: ${newConflict.title}`,
              timestamp: new Date().toLocaleTimeString()
            }
          ]);
        }
  ...
  useEffect(() => {
    ...
    // Update operational mode based on DCI
    if (telemetryLatency < 1500) {
      setOperationalMode(dciScore >= 60 ? 'AI_DRIVEN' : 'MANUAL_OVERRIDE');
    }
  ...
  const resolveConflict = (conflictId, resolution) => {
    setActiveConflicts(prev =>
      prev.map(conflict =>
        conflict.id === conflictId
          ? { ...conflict, resolved: true, chosenResolution: resolution }
          : conflict
      )
    );
    ...
    setSystemAlerts(alerts => [
      ...alerts,
      {
        id: Date.now(),
        type: 'success',
        message: `Conflict resolved: ${resolution === 'safety' ? 'Safety Override Applied' : 'AI Recommendation Accepted'}`,
        timestamp: new Date().toLocaleTimeString()
      }
    ]);

    // Auto-remove resolved conflicts after 5 seconds
    setTimeout(() => {
  ...
  const toggleHSEVeto = () => {
    ...
    setSystemAlerts(alerts => [
      ...alerts,
      {
        ...
        message: hseVetoLocked ? 'HSE Veto Released - Operations Enabled' : 'HSE VETO ACTIVATED - All Operations Halted',
        timestamp: new Date().toLocaleTimeString()
      }
    ]);
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`w-3 h-3 rounded-full animate-pulse ${
              operationalMode === 'AI_DRIVEN' ? 'bg-blue-400' :
              operationalMode === 'LOCAL_MODE' ? 'bg-red-400' :
              'bg-amber-400'
            }`} style={{
              boxShadow: `0 0 20px ${
                operationalMode === 'AI_DRIVEN' ? '#3b82f6' :
                operationalMode === 'LOCAL_MODE' ? '#ef4444' :
                '#f59e0b'
              }`
            }} />
          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              ...
              <div className="flex flex-col items-center justify-center h-40">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Radio className={`w-4 h-4 ${telemetryLatency < 1500 ? 'text-blue-400 animate-pulse' : 'text-slate-600'}`} />
              ...
              <div className="flex flex-col items-center justify-center h-40">
                ...
                >
                  {hseVetoLocked ? (
                    ...
                    <Unlock className="w-12 h-12 text-slate-400 mx-auto" />
                  )}
                </button>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div className="bg-slate-800 rounded p-3">
                        ...
                        <div className="text-sm text-blue-400 font-medium">{conflict.aiRecommendation}</div>
                      </div>
                      <div className="bg-slate-800 rounded p-3">
                        <div className="text-xs text-slate-400 mb-1">Safety Override</div>
                        <div className="text-sm text-amber-400 font-medium">{conflict.safetyOverride}</div>
                      </div>
                    </div>

                    <div className="flex items-center justify-between text-xs text-slate-400 mb-3">
                      <div className="flex items-center gap-4">
                        <span>Risk Score: <span className="text-red-400 font-semibold">{conflict.riskScore}/10</span></span>
                        <span>Performance Impact: <span className="text-amber-400 font-semibold">{conflict.performanceImpact}</span></span>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    {conflict.resolved && (
                      <div className="bg-green-900/30 border border-green-800 rounded p-2 text-sm text-green-400">
                        ✓ Resolved: {conflict.chosenResolution === 'safety' ? 'Safety Override Applied' : 'AI Recommendation Accepted'}
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {systemAlerts.length === 0 ? (
                ...
                systemAlerts.slice(-10).reverse().map(alert => (
                  <div
                    key={alert.id}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      {alert.type === 'warning' && <AlertOctagon className="w-4 h-4 flex-shrink-0 mt-0.5" />}
                      {alert.type === 'success' && <CheckCircle2 className="w-4 h-4 flex-shrink-0 mt-0.5" />}
                      <div className="flex-1">
